...
function Cover({ group, compact = false }: { group: RoutineGroup; compact?: boolean }) {
  ...
  const videoSrc = group.coverItem.videoSrc
  ...
  return (
    <div className={`relative overflow-hidden rounded-[18px] bg-black ${compact ? 'h-[132px]' : 'aspect-[1.05]'}`}>
      {videoSrc ? (
        ...
        <img className="h-full w-full object-cover transition duration-500 group-hover:scale-105" src={imageUrl(group.coverItem.imagePrompt)} alt={title} />
      )}
