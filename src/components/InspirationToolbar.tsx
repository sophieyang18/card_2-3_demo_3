...
export function InspirationToolbar() {
  ...
  const toggleFavoritesOnly = useWorkbenchStore((state) => state.toggleFavoritesOnly)
  ...
  return (
    <section className="sticky top-0 z-10 border-b border-white/6 bg-[#130f20]/72 px-6 pb-4 pt-5 backdrop-blur-xl">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <button
          className={`glass-pill h-10 px-4 text-sm ${filters.favoritesOnly ? 'text-white ring-[#7b5cff]/60' : 'text-[#cfc7df]'}`}
          onClick={toggleFavoritesOnly}
        >
          <Heart size={18} className={filters.favoritesOnly ? 'fill-[#ff6aa9] text-[#ff6aa9]' : ''} />
          我的收藏
        </button>
      </div>
