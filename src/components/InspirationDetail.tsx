    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#080610]/70 backdrop-blur-sm" onWheel={(event) => event.stopPropagation()}>
      <div className="absolute inset-x-5 bottom-5 top-5 flex min-h-0 flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[#171220]/96 shadow-[0_34px_120px_rgba(0,0,0,0.55)]">
        {isRoutineView ? (
          ...
          <DetailHeader title="素材详情" onClose={closeDetail} />
        )}
...
function DetailHeader({ title, onClose }: { title: string; onClose: () => void }) {
  return (
    <header className="flex h-[72px] flex-none items-center justify-between gap-4 border-b border-white/8 px-5">
      <button className="detail-icon-button" onClick={onClose} aria-label="返回灵感广场">
        <ArrowLeft size={18} />
      </button>
      <div className="min-w-0 flex-1 text-sm font-bold text-[#d8d1e8]">{title}</div>
      <button className="detail-icon-button" onClick={onClose} aria-label="关闭详情">
        <X size={18} />
      </button>
    </header>
  )
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="line-clamp-2 text-xs font-semibold leading-5 text-white">视频名称：{item.materialName ?? item.title}</div>
              <div className="mt-1 line-clamp-2 text-[11px] leading-4 text-[#bfb7d2]">{item.tertiaryRoutineName}</div>
    ...
    <aside className="min-h-0 overflow-y-auto border-r border-white/8 bg-[#100c19]/70 p-5">
      <div className="relative mx-auto aspect-[9/16] max-h-[390px] overflow-hidden rounded-[24px] border border-white/10 bg-black shadow-[0_22px_70px_rgba(0,0,0,0.45)]">
        ...
        <div className="pointer-events-none absolute bottom-4 left-4 right-4">
          ...
          <p className="mt-3 line-clamp-3 text-sm font-semibold leading-5 text-white">{item.tertiaryRoutineName}</p>
        </div>
      </div>
