...
export const inspirations: InspirationItem[] = rows.map(([rankText, secondaryRoutineName, tertiaryRoutineName, materialLink, materialName], index) => {
  ...
  const fallbackVideoSrc = fallbackVideos[index % fallbackVideos.length]

  return {
    id: `routine-top-${rankText}`,
    rank,
    audience: audienceLabels[index % audienceLabels.length],
    routineName: `routine-top-${rankText}`,
    secondaryRoutineName,
    tertiaryRoutineName,
    materialName,
    materialLink,
    videoSrc,
    detailVideoSrc: videoSrc ?? fallbackVideoSrc,
    title: `${tertiaryRoutineName}｜${secondaryRoutineName}`,
    heat: 1200 - index * 37,
