...
describe('selectVisibleItems', () => {
  it('加载 Excel top20 全量数据', () => {
    ...
    expect(inspirations[0]).toMatchObject({
      id: 'routine-top-1',
      materialName: '1',
      tertiaryRoutineName: '剧情·外观·信息·转化',
    })
  })
  ...
  it('支持关键词搜索不同层级的套路名称', () => {
    const tertiaryResult = selectVisibleItems(inspirations, { ...baseFilters, keyword: '剧情·外观·促销' })
    const secondaryResult = selectVisibleItems(inspirations, { ...baseFilters, keyword: '参数/信息说明›转化引导' })

    expect(tertiaryResult.map((item) => item.id)).toEqual(['routine-top-10'])
    expect(secondaryResult.map((item) => item.id)).toContain('routine-top-4')
  })
  ...
  it('支持按套路聚合并保留层级字段', () => {
    ...
    expect(groups[0].tertiaryRoutineName).toBe('剧情·外观·信息·转化')
